...
def test_ok(meeting: Meeting, invited_user: User, answer: bool):
    assert db.session.query(Invitation).filter_by(invitee=invited_user, meeting=meeting).first().answer is None
    set_answer_for_invitation(invitee=invited_user, meeting=meeting, answer=answer)
    assert db.session.query(Invitation).filter_by(invitee=invited_user, meeting=meeting).first().answer is answer
